...
{
    public partial class TranspoExpenses : Form
    {
        public TranspoExpenses()
        {
            InitializeComponent();
            SQLMethods.InitializeDataGridView(dg_TranspoExpense, ColorTranslator.FromHtml("#4e2a84"));
        }
        MySQLMethods SQLMethods = new MySQLMethods();
        private String Table = "transpo_expense";

        private void TranspoExpenses_Load(object sender, EventArgs e)
        {
            SQLMethods.LoadAllData("transpo_expense", dg_TranspoExpense);
        ...
        private void pb_TitleSearch_Click(object sender, EventArgs e)
        {
            ...
            SQLMethods.SearchTitle(Table, SearchedTitle, dg_TranspoExpense);
        }
        ...
        private void pb_DateSearch_Click(object sender, EventArgs e)
        {
            ...
            SQLMethods.SearchDate(Table, Date.ToString("yyyy-MM-dd"), dg_TranspoExpense);
        }
        ...
        {
            String Table = "transpo_expense";
            String Title = txt_TitleSearch.Text;
            //If Both Title and Date has no input
            if ((Title.Equals("(e.g. \"Title\")") || Title.Equals("")))
            {
                String Query = "Select * FROM transpo_expense ORDER BY Date DESC";
                SQLMethods.OrderedData(Query, dg_TranspoExpense);
                return;
            }
            //If Title has input
            if (!(Title.Equals("(e.g. \"Title\")") || Title.Equals("")))
            {
                ...
                SQLMethods.OrderedDataWithTitleByDate(Table, SearchedTitle, dg_TranspoExpense, "DESC");
                return;
            }
        ...
        private void rb_EarliestFirst_CheckedChanged(object sender, EventArgs e) // this function doesnt read the date for it arranges them by date
        {
            ...
            if ((Title.Equals("(e.g. \"Title\")") || Title.Equals("")))
            {
                String Query = "Select * FROM transpo_expense ORDER BY Date ASC";
                SQLMethods.OrderedData(Query, dg_TranspoExpense);
                return;
            }
            //If Title has input
            if (!(Title.Equals("(e.g. \"Title\")") || Title.Equals("")))
            {
                ...
                SQLMethods.OrderedDataWithTitleByDate(Table, Searched, dg_TranspoExpense, "ASC");
                return;
            }
        }
        ...
        private void rb_AtoZ_CheckedChanged(object sender, EventArgs e)//this function only depends on the date
        {
            ...
            {
                String Query = "Select * FROM transpo_expense Order By Title ASC";
                SQLMethods.OrderedData(Query, dg_TranspoExpense);
                return;
            }
            SQLMethods.OrderedDataWithDateByTitle("transpo_expense", Date, dg_TranspoExpense, "ASC");
        }
        ...
        private void rb_ZtoA_CheckedChanged(object sender, EventArgs e) //this function only depends on the date
        {
            ...
            if (Date.Equals("YYYY-MM-DD") || Date.Equals(""))
            {
                String Query = "Select * FROM transpo_expense Order By Title DESC";
                SQLMethods.OrderedData(Query, dg_TranspoExpense);
                return;
            }
            SQLMethods.OrderedDataWithDateByTitle("transpo_expense", Date, dg_TranspoExpense, "DESC");
        }
        ...
        private void rb_HighestPrice_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                String Query = "Select * FROM transpo_expense Order By Price DESC";
                SQLMethods.OrderedData(Query, dg_TranspoExpense);
                return;
            }
            // if title is empty
            ...
            {
                SQLMethods.OrderedDataWithDateByPrice("transpo_expense", Date, dg_TranspoExpense, "DESC");
            }
            // if Date is empty
            if (!(Title.Equals("") || Title.Equals("(e.g. \"Title\")")) && (Date.Equals("YYYY-MM-DD") || Date.Equals("")))
            {
                SQLMethods.OrderedDataWithTitleByPrice("transpo_expense", Title, dg_TranspoExpense, "DESC");
            }
            // if both is not empty
            if (!(Title.Equals("") || Title.Equals("(e.g. \"Title\")")) && !(Date.Equals("YYYY-MM-DD") || Date.Equals("")))
            {
                SQLMethods.OrderedDateAndTitleByPrice("transpo_expense", Title, Date, dg_TranspoExpense, "DESC");
            }
        }
        ...
        private void rb_LowestPrice_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if ((Date.Equals("YYYY-MM-DD") || Date.Equals("")) && (Title.Equals("") || Title.Equals("(e.g. \"Title\")")))
            {
                String Query = "Select * FROM transpo_expense Order By Price ASC";
                SQLMethods.OrderedData(Query, dg_TranspoExpense);
                return;
            }
            // if title is empty
            if ((Title.Equals("") || Title.Equals("(e.g. \"Title\")")) && !(Date.Equals("YYYY-MM-DD") || Date.Equals("")))
            {
                SQLMethods.OrderedDataWithDateByPrice("transpo_expense", Date, dg_TranspoExpense, "ASC");
            }
            // if Date is empty
            if (!(Title.Equals("") || Title.Equals("(e.g. \"Title\")")) && (Date.Equals("YYYY-MM-DD") || Date.Equals("")))
            {
                SQLMethods.OrderedDataWithTitleByPrice("transpo_expense", Title, dg_TranspoExpense, "ASC");
            }
            // if both is not empty
            if (!(Title.Equals("") || Title.Equals("(e.g. \"Title\")")) && !(Date.Equals("YYYY-MM-DD") || Date.Equals("")))
            {
                SQLMethods.OrderedDateAndTitleByPrice("transpo_expense", Title, Date, dg_TranspoExpense, "ASC");
            }
        }

        private void dg_TranspoExpense_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            {
                string Description = dg_TranspoExpense.Rows[e.RowIndex].Cells["Description"].Value.ToString();
                string Date = dg_TranspoExpense.Rows[e.RowIndex].Cells["Date"].Value.ToString();
                string Title = dg_TranspoExpense.Rows[e.RowIndex].Cells["Title"].Value.ToString();
                ...
                {
                    SQLMethods.Delete("transpo_expense", Title, Format.ToString("yyyy-MM-dd"));
                    SQLMethods.LoadAllData("transpo_expense", dg_TranspoExpense);
                }
            }
        }
